...
- [测试] 本轮 Web 方向验证执行：`cd apps/dsa-web && npm run lint`、`cd apps/dsa-web && npm run build`、`cd apps/dsa-web && npm run test -- src/App.test.tsx src/contexts/__tests__/UiLanguageContext.test.tsx src/hooks/__tests__/useSystemConfig.test.tsx`、`cd apps/dsa-web && npm run test:smoke -- e2e/smoke.spec.ts e2e/report-markdown.spec.ts`（均通过；当前环境 `DSA_WEB_SMOKE_PASSWORD` 未设置导致 12 个 `smoke` 用例跳过）。
