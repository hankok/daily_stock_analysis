...
- [文档] 明确 `SCHEDULE_TIME` 与 GitHub Actions 固定 `cron` 的边界：`SCHEDULE_TIME` 仅影响本地内置调度（`python main.py --schedule`），仓库默认 `00-daily-analysis.yml` 的触发时点仍由固定 `cron` 决定（周一到周五 10:00 UTC / 北京 18:00）；本次为 docs-only 说明文档范围，不包含运行时能力交付。
