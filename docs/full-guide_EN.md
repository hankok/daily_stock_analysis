...
> - CN market review reports now use a post-market workstation layout with fixed market signal, market temperature, index detail, sector Top tables, news catalysts, next-session plan, and risk sections. The market signal only shows the light and trading stance, while market temperature keeps the single score and bar. Missing data sources degrade by omitting or simplifying only the affected block.
