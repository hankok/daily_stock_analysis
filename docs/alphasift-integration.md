...
- 依赖来源：`requirements.txt` 固定到已验证的 AlphaSift 适配层 commit：`git+https://github.com/ZhuLinsen/alphasift.git@b2ca66dd47001b9a09890cfe21c2b18c7219ccf5#egg=alphasift`。该来源覆盖 `alphasift.dsa_adapter` 契约与 `screen/list_strategies/get_status` 调用。
...
- 策略归属：策略列表、策略参数、全市场快照、初筛、因子评分和 LLM 重排由 AlphaSift 负责；DSA 负责开关、API 壳、候选增强、展示和错误提示。
- DSA 增强：AlphaSift 返回候选后，DSA 对 Top N 候选补充实时行情、股票名称、基本面上下文、新闻结果和辅助摘要，并追加到 `dsa_context`、`dsa_news`、`dsa_analysis_summary`、`dsa_enrichment` 字段。
...
AlphaSift 可以选择在 LLM 重排前调用 `context["dsa"]` 中的 provider；即使 AlphaSift 暂不消费该上下文，DSA 也会在 API 返回前对 Top N 候选做后置增强。
...
- `/api/v1/alphasift/screen`：调用适配层 `screen(..., use_llm=True)`，并在调用期间临时注入 DSA 已解析的 LLM 运行环境，同时向支持 `context` 的适配层传入结构化 LLM/DSA provider 配置；AlphaSift 返回后，DSA 对 Top N 候选执行后置增强。适配层缺失或运行时异常返回 `424 + diagnostics` 并保留原始错误边界。
