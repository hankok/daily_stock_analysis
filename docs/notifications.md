...
聚合股票报告在企业微信、飞书、Telegram 和 Slack 文本路径会先生成 IM 专用报告：同源使用完整 `AnalysisResult`，但把表格压成短行，把每只股票拆成「结论速览 / 数据透视 / 操作建议 / 关键点位 / 仓位与风控 / 风险机会 / 检查清单」等聊天窗口更易扫读的块。IM 报告正文不使用 `|` / `｜` 做表格式分隔；企业微信和 Telegram 使用短横列表，飞书和 Slack 使用圆点列表，并按平台分别生成文本。邮件、自定义 Webhook、ntfy、Gotify 等渠道仍使用原完整 Markdown 报告；IM 报告生成异常时会自动回退原报告，不影响通知主流程。
