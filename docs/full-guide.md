...
> 本节为边界说明（仅文档说明，不包含 Actions 触发机制改造）。`SCHEDULE_TIME` 仅影响本地内置调度（`python main.py --schedule`），仓库默认 `00-daily-analysis.yml` 的执行时点仍由固定 `cron` 决定（周一到周五 10:00 UTC / 北京 18:00）。
...
该说明为文档边界澄清，仅限阐明当前行为，不代表已交付 Actions runtime 动态触发能力。
> 该说明为文档边界澄清，动态触发能力需通过后续 PR 补齐；当前仅同步文档说明，不应视为运行时交付或 PR 自动关闭依据（文档类 PR 请勿使用 `Closes` / `Fixes`）。
